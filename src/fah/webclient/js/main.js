...
function update_stats(data) {
  ...
  $("#box-points-counter-contributed").text(
    number_with_commas(data.contributed)
  );
  ...
  if (fah.team) {
    ...
    if (typeof data.team_url != "undefined")
      team_name =
        '<a target="_blank" href="' + data.team_url + '">' + team_name + "</a>";
...
function update_basic(data) {
  ...
  if (
    fah.f_user != data.user ||
    fah.team != data.team ||
    fah.passkey != data.passkey
  )
    fah.last_stats = 0;
...
function set_status_msg(status, reason) {
  ...
  if (typeof reason != "undefined" && reason != "" && status == "paused")
    status_msg = reason;
  else if (status in fah.status_msg) status_msg = fah.status_msg[status];
  else status_msg = "";

  $("#box-status-msg").html(
    '<span class="hlarge">' + status + "</span>" + status_msg
  );
}
...
function activate_slot(slot) {
  ...
  $("#box-status-time").removeClass("unknown");
...
function update_slot(entry) {
  ...
  if (entry.percentdone !== slot.percentdone) {
    slot.tab
      ...
      .progressbar({
        value: parseInt(
          entry.percentdone.substr(0, entry.percentdone.length - 1)
        )
      })
  ...
  if (entry.creditestimate !== slot.creditestimate) {
    if (is_active_slot(id)) $("#box-stats-points").text(entry.creditestimate);
...
function is_active_project(id) {
  return (
    typeof fah.active_slot != "undefined" &&
...
function update_project(p) {
  ...
  if (typeof p.error != "undefined") {
    ...
    project =
      '<p>I\'m contributing to <span class="hlarge">Project ' +
      p.id +
      "</span></p>" +
      pbrief +
      ' <a href="javascript:void(0)" ' +
      'onclick="show_project_description(' +
      p.id +
      '); return false;">' +
      "Learn more</a>";
    ...
    description =
      ...
      p.pdesc +
      "<br/>" +
      "<strong>This project is managed by " +
      p.name +
      " at " +
      p.uni +
      ".</strong>" +
...
function add_project(id) {
  ...
  if (is_active_project(id)) $("#box-project").html("Loading...");
...
function updates(now) {
  if (
    fah.disconnected ||
    now < fah.last_update + 1000 ||
    1 < fah.active_updates
  )
    return;
...
var fah_create_client = (function() {
  function fah_create_client(opts) {
    ...
    else if (browser == "Mozilla" && 11 <= version);
    else {
      // IE in disguise
      $("#browser-warn").dialog({
        title: "Warning: Browser reports as " + browser + " version " + version,
        modal: true,
        resizable: false,
        width: 600,
        buttons: {
    ...
    if (typeof sid === "undefined" || sid === "") {
      debug("invalid session ID, reloading page in 10 seconds");
      setTimeout(function () {
        location.reload(true);
      }, 10000);
      return;
    ...
    $(document).hover(
      function() {
        fah.mouse_active = true;
      },
      function() {
        fah.mouse_active = false;
      }
    );
    ...
    setInterval(function() {
      ...
      if (
        (fah.mouse_active && fah.last_mouse + fah.mouse_inside_timeout < now) ||
        (!fah.mouse_active && fah.last_mouse + fah.mouse_outside_timeout < now)
      ) {
        fade_screen(true);
        return;
      }
  ...
  fah_create_client.prototype.disconnect = function() {
    disconnect();
  };
  ...
  fah_create_client.prototype.set_identity = function(
    new_user,
    new_passkey,
    new_team
  ) {
    ...
    if (new_user !== fah.user) {
      if (!/^[!-~]+$/.test(new_user)) {
        alert(
          "User name must be a non-empty string containing " +
    ...
    if (new_passkey !== fah.passkey) {
      if (new_passkey != "" && !/^[a-fA-F0-9]{32}$/.test(new_passkey)) {
    ...
    if (!/^\d{1,10}$/.test(new_team) || $.isNumeric(new_team) == false) {
      alert(
        "Team number must be a whole number no more than ten " + "digits long."
      );
      return false;
    }
...
$(function() {
  ...
  $("#setupid-btn-save").on("click", function(e) {
    e.preventDefault();
    if (set_identity()) $("#setupid-popup").dialog("destroy");
  });
  ...
  $("#idle").change(function(e) {
    set_idle($(this).is(":checked"));
  });
  $("#not_idle").change(function(e) {
    set_idle(!$(this).is(":checked"));
  });
  ...
  var body =
    "Here is a site where we can share our unused computer power " +
  ...
  $("a.email").attr({
    href: "mailto:?subject=" + subject + "&body=" + body.replace("\n", "%0A"),
    rel: "nofollow",
    title:
      "Click here to open your email and send this message to your " +
      "friends:\n\n---------------------------------------------------" +
      "-----------------------------------------------\nSubject: " +
      subject +
      "\n\n" +
